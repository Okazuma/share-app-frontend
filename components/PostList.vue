            ...
            <div class="border-b-2 border-gray-300 p-2 my-2 " v-for="post in postStore.posts" :key="post.id">
                <div class="buttons mb-2">
                    ...
                    <button @click="toggleLike(post.id)" :style="{color: post.isLiked ? 'red' : 'white'}">
                        <svg id="svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="500" height="500" viewBox="0, 0, 500,500" class="w-6 h-6 inline-block ml-4" alt="ハートアイコン">
                        <g id="svgg">
                    ...
                    <!-- いいねカウント -->
                    <span class="text-white ml-2">{{ post.likes }}</span>

                    <!-- コメントへ -->
                    <NuxtLink :to="`/posts/${post.id}`">
                        <img src="/images/detail.png" class="w-6 h-6 inline-block ml-4" alt="詳細" />
                    </NuxtLink>
                    ...
                    <button v-if="post.user_id === currentUserId" @click="deletePost(post.id)">
                        <img src="/images/cross.png" class="w-6 h-6 inline-block ml-8" alt="詳細" />
                    </button>
                    <button>
                        <i class="fa-solid fa-pen-nib text-white w-6 h-6 inline-block ml-8"></i>
                    </button>
                </div>
            <span class="block text-white ml-2 "><p>{{ post.content }}</p></span>
            <small class="text-white ml-2">{{ formatDate(post.created_at) }}</small>
            </div>
        </form>
