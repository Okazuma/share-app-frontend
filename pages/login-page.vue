<template>
    <section class="w-screen h-screen bg-indigo-950">
        ...
        <form @submit.prevent="login" class="bg-white mx-auto mt-12 p-4 w-80">
            ...
            <input v-model="email" type="emial" name="email" placeholder="メールアドレス" class="block my-4 mx-auto border-2 border-gray-300 p-1 rounded">
...
const login = async () => {
    ...
    try {
        // Firebase認証を利用してログイン
        await userStore.login($auth, email.value, password.value);
        alert("ログイン成功！");
